...
st.markdown("""
    <style>
        ...
        .audit-dashboard {
            background: linear-gradient(to right, #fff7ed, #fff); border: 1px solid #ffedd5; border-radius: 12px; padding: 20px; margin-bottom: 20px;
        }
...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        with c2:
            ...
            st.markdown("<div style='text-align:center; color:#666; font-size:0.9rem; margin-bottom:30px;'>MATRIX ERP V53.0</div>", unsafe_allow_html=True)
            with st.form("login"):
    ...
    # --- Tabs ---
    tabs = st.tabs(["📊 視覺庫存", "⚡ POS (精準版)", "🎁 內部領用", "👔 矩陣管理", "📝 日誌", "👥 Admin"])
    ...
    with tabs[1]:
        ...
        with c1:
            ...
            # V53: 改為 SKU 級別選擇
            if not df.empty:
                # 建立詳細選單: SKU | Name (Size)
                sku_opts = df.apply(lambda x: f"{x['SKU']} | {x['Name']} ({x['Size']}) | 現貨:{x['Qty']}", axis=1).tolist()
            else: sku_opts = []
            
            sel_sku = st.selectbox("搜尋商品 (支援條碼/品名)", ["..."] + sku_opts, key="pos_sku_sel")
        ...
        with c2:
            ...
            if target is not None:
                ...
                t1, t2 = st.tabs(["📥 進貨 (Restock)", "📤 銷售 (Sell)"])
    ...
    # Tab 3: Internal (V52.0 全域稽核版)
    with tabs[2]:
        st.subheader("🎁 內部領用中心")
        logs_df = get_data_safe(ws_logs)
        usage_count = 0; total_items_taken = 0; int_logs = pd.DataFrame()
        if not logs_df.empty:
            ...
            for _, r in int_logs.iterrows():
                try:
                    ...
                    for p in parts:
                        if p.startswith('-') and p[1:].isdigit(): total_items_taken += int(p[1:])
                except: pass
        
        st.markdown(f"""
        <div class="audit-dashboard">
            <div style="display:flex; justify-content:space-around;">
                <div style="text-align:center;"><div class="audit-title">本月領用筆數</div><div class="audit-stat">{usage_count}</div></div>
                <div style="text-align:center;"><div class="audit-title">累計領用件數</div><div class="audit-stat">{total_items_taken}</div></div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with c_i1:
            st.markdown("##### 1. 選擇商品 (SKU)")
            if not df.empty:
                sku_opts = df.apply(lambda x: f"{x['SKU']} | {x['Name']} ({x['Size']}) | 現貨:{x['Qty']}", axis=1).tolist()
            else: sku_opts = []
            sel_int_sku = st.selectbox("搜尋具體款式", ["..."] + sku_opts, key="int_sel_v52")
            t_int = None
            if sel_int_sku != "...":
            ...
            if t_int is not None:
                with st.form("int_form_v52"):
                    ...
                    if st.form_submit_button("確認領用 (扣除庫存)", type="primary"):
                        current_q = int(t_int['Qty'])
                        if current_q >= iq:
                            r = ws_items.find(t_int['SKU']).row
                            ws_items.update_cell(r, 5, current_q - iq)
                            log_detail = f"{t_int['SKU']} -{iq} | {who} | {rsn} | {int_note}"
                            log_event(ws_logs, st.session_state['user_name'], "Internal_Use", log_detail)
                            st.success(f"✅ 成功扣除！庫存 {current_q} -> {current_q - iq}"); time.sleep(1); st.rerun()
                        else: st.error("庫存不足！")

        st.divider()
        st.markdown("#### 👁️ 全域領用/報廢總覽 (Global Audit)")
        if not int_logs.empty:
            ...
            for i, row in int_logs.iterrows():
                ...
                sku_pure = sku_qty.split(' ')[0]; qty_pure = sku_qty.split(' ')[1] if len(sku_qty.split(' '))>1 else "?"
                display_data.append({"Timestamp": row['Timestamp'], "SKU": sku_pure, "Qty": qty_pure, "User": user_log, "Reason": reason_log})
            audit_df = pd.DataFrame(display_data)
            st.dataframe(audit_df[['Timestamp', 'SKU', 'Qty', 'User', 'Reason']], use_container_width=True)
            
            st.markdown("##### 🛠️ 強制回溯操作")
            rev_options = audit_df.apply(lambda x: f"{x['Timestamp']} | {x['SKU']} {x['Qty']} ({x['Reason']})", axis=1).tolist()
            sel_rev_target = st.selectbox("選擇要撤銷/刪除的紀錄", ["..."] + rev_options)
            if sel_rev_target != "...":
                target_timestamp = sel_rev_target.split(" | ")[0]
                target_sku = sel_rev_target.split(" | ")[1].split(" ")[0]
                c_btn1, c_btn2 = st.columns(2)
                with c_btn1:
                    if st.button("🚫 撤銷並歸還庫存"):
                        all_logs_data = ws_logs.get_all_values()
                        log_row_num = -1
                        for idx, row in enumerate(all_logs_data):
                            if row[0] == target_timestamp and target_sku in row[3]: log_row_num = idx + 1; break
                        if log_row_num != -1:
                            item_cell = ws_items.find(target_sku)
                            if item_cell:
                                try:
                                    qty_part = sel_rev_target.split(" | ")[1].split(" ")[1]; restore_q = abs(int(qty_part))
                                    curr_q = int(ws_items.cell(item_cell.row, 5).value)
                                    ws_items.update_cell(item_cell.row, 5, curr_q + restore_q)
                                    ws_logs.delete_rows(log_row_num)
                                    st.success(f"✅ 已歸還 {target_sku} +{restore_q}，並刪除紀錄。"); time.sleep(1); st.rerun()
                                except: st.error("數量解析失敗。")
                            else: st.error("❌ 商品已不存在。")
                        else: st.error("❌ 找不到原始 Log。")
                with c_btn2:
                    if st.button("🗑️ 僅刪除紀錄"):
                        all_logs_data = ws_logs.get_all_values()
                        log_row_num = -1
                        for idx, row in enumerate(all_logs_data):
                            if row[0] == target_timestamp and target_sku in row[3]: log_row_num = idx + 1; break
                        if log_row_num != -1: ws_logs.delete_rows(log_row_num); st.warning("⚠️ 已刪除紀錄。"); time.sleep(1); st.rerun()
        else: st.info("目前無任何領用紀錄。")

    # Tab 4: Mgmt (V49+V48)
    with tabs[3]:
        mt2, mt3, mt4 = st.tabs(["➕ 單品/全系列新增 (智能版)", "🛠️ 貨號重鑄", "🗑️ 刪除中心"])
        with mt2:
                        ...
                        for size, qty in size_inputs.items():
                            if qty > 0:
                                ...
                                if full_sku in df['SKU'].tolist():
                                    ...
                                    ws_items.update_cell(r, 5, current_q_val + qty); ws_items.update_cell(r, 8, str(datetime.now()))
        ...
        with mt3:
            st.markdown("<div class='refactor-zone'><div class='refactor-header'>🛠️ 貨號重鑄</div>", unsafe_allow_html=True)
            if not df.empty: style_opts = df[['Style_Code', 'Name']].drop_duplicates(subset=['Style_Code', 'Name']).apply(lambda x: f"{x['Style_Code']} | {x['Name']}", axis=1).tolist()
            else: style_opts = []
            ...
            if target_sel != "...":
                ...
                if st.button("☣️ 執行重鑄遷移", type="primary", disabled=not new_base_code):
                    try:
                        ...
                        st.success("✅ 遷移完成！"); time.sleep(1); st.rerun()
        ...
        with mt4:
            ...
            if del_mode == "單品刪除":
                ...
                if d_style_sel != "...":
                    ...
                    st.dataframe(to_delete_df[['SKU', 'Name', 'Size']])
    ...
    with tabs[5]:
        if st.session_state['user_role'] == 'Admin':
            st.subheader("👥 人員管理")
            users_df = get_data_safe(ws_users); st.dataframe(users_df, use_container_width=True)
