...
st.markdown("""
    <style>
        ...
        .usage-dashboard {
            background: #fff7ed; border: 1px solid #ffedd5; border-radius: 12px; padding: 15px; margin-bottom: 20px;
        }
...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        with c2:
            ...
            st.markdown("<div style='text-align:center; color:#666; font-size:0.9rem; margin-bottom:30px;'>MATRIX ERP V51.0</div>", unsafe_allow_html=True)
            with st.form("login"):
    ...
    with tabs[2]:
        ...
        if not logs_df.empty:
            ...
            # 解析細節以計算數量 (Log格式: SKU -Qty | User)
            # 這裡做簡單統計：筆數
            usage_count = len(int_logs)
            # 嘗試計算總件數 (進階)
            total_items_taken = 0
            for _, r in int_logs.iterrows():
                try:
                    # 嘗試解析 -qty
                    parts = r['Details'].split(' ')
                    for p in parts:
                        if p.startswith('-') and p[1:].isdigit():
                            total_items_taken += int(p[1:])
                except: pass
        ...
        <div class="usage-dashboard">
            <div style="display:flex; justify-content:space-around;">
                <div style="text-align:center;">
                    <div style="font-size:0.8rem; color:#666;">本月領用筆數</div>
                    <div style="font-size:1.5rem; font-weight:bold; color:#d97706;">{usage_count}</div>
                </div>
                <div style="text-align:center;">
                    <div style="font-size:0.8rem; color:#666;">累計領用件數</div>
                    <div style="font-size:1.5rem; font-weight:bold; color:#b45309;">{total_items_taken if 'total_items_taken' in locals() else '-'}</div>
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with c_i1:
            ...
            sel_int_sku = st.selectbox("搜尋具體款式 (SKU)", ["..."] + sku_opts, key="int_sel_v51")
            
            t_int = None
            if sel_int_sku != "...":
                target_sku = sel_int_sku.split(" | ")[0]
                t_int = df[df['SKU'] == target_sku].iloc[0]
                
                # 顯示卡片確認
                img = render_image_url(t_int['Image_URL'])
                st.markdown(f"""
                <div style="border:1px solid #ddd; border-radius:8px; padding:10px; display:flex; align-items:center; background:#fff;">
                    <img src="{img}" style="width:60px; height:60px; object-fit:cover; border-radius:4px; margin-right:10px;">
                    <div>
                        <div style="font-weight:bold;">{t_int['Name']}</div>
                        <div style="font-family:monospace; color:#555;">{t_int['SKU']}</div>
                        <div style="color:#d32f2f; font-weight:bold;">現貨: {t_int['Qty']}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with st.form("int_form_v51"):
                    ...
                    if st.form_submit_button("確認領用 (扣除庫存)", type="primary"):
                        if int(t_int['Qty']) >= iq:
                            ...
                            # 紀錄 Log: SKU -Qty | User | Reason
                            log_detail = f"{t_int['SKU']} -{iq} | {who} | {rsn} | {int_note}"
                            log_event(ws_logs, st.session_state['user_name'], "Internal_Use", log_detail)
                            st.success(f"✅ 領用成功！已扣除 {iq} 件。"); time.sleep(2); st.rerun()
                        else:
                            st.error("庫存不足！")

        st.divider()
        st.markdown("#### ↩️ 歷史紀錄與回溯 (History & Revert)")
        
        if not logs_df.empty:
            # 篩選並顯示最近 20 筆領用
            int_logs_disp = logs_df[logs_df['Action'] == 'Internal_Use'].sort_index(ascending=False).head(20)
            
            if not int_logs_disp.empty:
                # 製作一個選擇選單讓用戶挑選要回溯的項目
                # 格式: [時間] SKU -Qty (User)
                revert_opts = int_logs_disp.apply(lambda x: f"{x['Timestamp']} | {x['Details']}", axis=1).tolist()
                
                c_rev1, c_rev2 = st.columns([3, 1])
                with c_rev1:
                    sel_revert = st.selectbox("選擇要撤銷的紀錄", ["..."] + revert_opts)
                with c_rev2:
                    if sel_revert != "...":
                        if st.button("🚫 撤銷並歸還庫存"):
                            # 解析 Log
                            # Timestamp | SKU -Qty | User ...
                            try:
                                log_ts = sel_revert.split(" | ")[0]
                                log_detail = sel_revert.split(" | ")[1] # SKU -Qty
                                
                                # 找出 log 在 sheet 中的行號 (利用 Timestamp 和 Details 匹配)
                                # 注意：這有風險如果完全重複，但機率低
                                cell = ws_logs.find(log_ts) 
                                # 這裡簡化：假設 Timestamp 唯一。更嚴謹應用 row index mapping
                                # 我們用 DataFrame 的 index 反查
                                # 為了安全，我們遍歷 ws_logs 倒序查找
                                all_logs = ws_logs.get_all_values()
                                target_row_idx = -1
                                for i, row in enumerate(all_logs):
                                    if row[0] == log_ts and row[3] in log_detail: # row[3] is Details
                                        target_row_idx = i + 1
                                        break
                                
                                if target_row_idx != -1:
                                    # 解析 SKU 和 Qty
                                    # log_detail 格式: "TOP-2601-S -1 | ..."
                                    part1 = log_detail.split(" | ")[0] # "TOP-2601-S -1"
                                    sku_rev = part1.split(" ")[0]
                                    qty_rev_str = part1.split(" ")[1] # "-1"
                                    qty_restore = abs(int(qty_rev_str))
                                    
                                    # 歸還庫存
                                    item_cell = ws_items.find(sku_rev)
                                    if item_cell:
                                        curr_q = int(ws_items.cell(item_cell.row, 5).value)
                                        ws_items.update_cell(item_cell.row, 5, curr_q + qty_restore)
                                        
                                        # 刪除 Log
                                        ws_logs.delete_rows(target_row_idx)
                                        
                                        st.success(f"已撤銷！庫存 +{qty_restore}，紀錄已刪除。")
                                        time.sleep(2); st.rerun()
                                    else:
                                        st.error("找不到該商品，無法歸還庫存 (可能已被刪除)。")
                                else:
                                    st.error("找不到原始紀錄。")
                ...
                # 顯示列表
                st.dataframe(int_logs_disp[['Timestamp', 'User', 'Details']], use_container_width=True)
            else:
                st.info("尚無領用紀錄。")
        ...
        with mt2:
            ...
            if "開闢新系列" in gen_mode:
                ...
                if sel_parent != "..." and suffix_code:
                    p_code = sel_parent.split(" | ")[0]; p_name = sel_parent.split(" | ")[1]
                    auto_sku = f"{p_code}-{suffix_code}"; auto_name = p_name
                    try:
                        p_row = df[(df['Style_Code'] == p_code) & (df['Name'] == p_name)].iloc[0]
                        auto_img = p_row['Image_URL']; inherit_price = int(p_row['Price'])
                        inherit_cost = int(p_row['Orig_Cost']) if p_row['Orig_Currency'] == 'CNY' else int(p_row['Cost'])
                        inherit_curr = p_row['Orig_Currency']; inherit_cat = p_row['Category']
                        st.info(f"🧬 已繼承 [{p_code}]。")
                    except: pass
                 ...
                 if sel_p != "...":
                     p_c = sel_p.split(" | ")[0]; p_n = sel_p.split(" | ")[1]
                     auto_sku = p_c; auto_name = p_n
                     try: 
            ...
            with st.form("matrix_add_v48"):
                ...
                sku_val = auto_sku if auto_sku else ""; name_val = auto_name if auto_name else ""
                base_sku_input = c_sa.text_input("基礎貨號 (Base SKU)", value=sku_val)
                ...
                if "開闢新系列" in gen_mode and base_sku_input:
                    ...
                    if not conflict_check.empty:
                        ...
                        if exist_name != name_input: st.warning(f"⚠️ 警告：貨號已被 [{exist_name}] 佔用。")
                ...
                st.markdown("---")
                st.markdown("<div class='batch-title'>🎹 尺寸庫存網格</div>", unsafe_allow_html=True)
                size_inputs = {}
                        ...
                        for size, qty in size_inputs.items():
                            if qty > 0:
                                ...
                                if full_sku in df['SKU'].tolist():
                                    ...
                                    ws_items.append_row([full_sku, name_input, cat_input, size, qty, price_input, final_cost_val, str(datetime.now()), final_img_payload, 5, curr_input, cost_input])
            ...
            if del_mode == "單品刪除 (Single SKU)":
                ...
                if d_sku_sel != "...":
                    if st.button("🚫 執行刪除", type="primary"):
                        try:
                            cell = ws_items.find(d_sku_sel); ws_items.delete_rows(cell.row)
                    ...
                    if st.button("☢️ 執行全款刪除", type="primary"):
                        try:
                            ...
                            for idx, row in to_delete_df.iterrows():
                                cell = ws_items.find(row['SKU']); rows_to_del.append(cell.row)
                            rows_to_del.sort(reverse=True)
                            for r_idx in rows_to_del: ws_items.delete_rows(r_idx)
                            log_event(ws_logs, st.session_state['user_name'], "Delete_Style", f"Deleted: {target_code}")
