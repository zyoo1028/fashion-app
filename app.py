                        ...
                        for size, qty in size_inputs.items():
                            if qty > 0:
                                ...
                                if full_sku in df['SKU'].tolist():
                                    ...
                                    update_cell_retry(ws_items, r, 5, current_q_val + qty)
                                    update_cell_retry(ws_items, r, 8, get_taiwan_time_str())
                                    update_cell_retry(ws_items, r, 2, name_input)
                                    update_cell_retry(ws_items, r, 6, price_input)
                                    if final_img_payload: update_cell_retry(ws_items, r, 9, final_img_payload)
