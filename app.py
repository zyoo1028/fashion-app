...
# --- 1. 系統全域設定 (已更改標題) ---
# 這裡也同步改成新名稱，讓瀏覽器分頁標題也變更
st.set_page_config(page_title="IFUKUK 庫存系統", layout="wide", page_icon="✨")
...
def init_db():
    ...
    try:
        # ⚠️⚠️⚠️ 主理人：請務必確認您的 Google 試算表網址是否正確 ⚠️⚠️⚠️
...
def main():
    ...
    if not st.session_state['logged_in']:
        ...
        with c2:
            ...
            # ▼▼▼ 修改重點在這裡 ▼▼▼
            # 使用了新的 class 'login-title' 來套用字體樣式
            st.markdown("<h1 class='login-title'>IFUKUK庫存系統</h1>", unsafe_allow_html=True)
    ...
    m3.metric("💰 庫存市值", f"${total_revenue:,.0f}")
    m4.metric("📈 潛在利潤", f"${potential_profit:,.0f}", delta_color="normal")
