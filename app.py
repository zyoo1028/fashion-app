...
# --- 1. 系統全域設定 ---
st.set_page_config(
    page_title="IFUKUK V103.22 ULTIMATE REBORN", 
    layout="wide", 
    page_icon="🌏",
...
st.markdown("""
    <style>
        ...
        .shift-tag { 
            font-size: 0.85rem; padding: 4px 8px; border-radius: 6px; margin: 4px 0; 
            display: block; text-align: center; color: white; font-weight: bold; 
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not df.empty:
                ...
                if "Qty_CN" in expected_headers and "Qty_CN" not in df.columns: df["Qty_CN"] = "0"
...
def render_roster_system(sh, users_list):
    ...
    # 1. 批次排班 (回歸)
    with st.expander("⚡ 智慧批次排班 (多日連排)", expanded=True):
...
def main():
    ...
    # [V103.22 Fix] 初始化所有分頁變數，防止 KeyError
        ...
        with c_r:
            ...
            with st.container():
                ...
                if st.session_state['pos_cart']:
                    ...
                    # [V103.22 Fix] 組合價手動輸入框回歸
                    use_bundle = st.checkbox("啟用組合價 (手動輸入總額)")
    ...
    # --- 5. 領用 ---
        ...
        if not int_logs.empty:
            ...
            st.markdown(f"""<div style='display:flex; gap:10px;'><div class='audit-stat-box' style='flex:1;'><div class='audit-num'>{total_int}</div><div class='audit-txt'>總領用筆數</div></div></div>""", unsafe_allow_html=True)
            ...
            if 'int_t' in st.session_state:
                ...
                with st.form("int_do"):
                    ...
                    if st.form_submit_button("執行"):
                        ...
                        log_event(ws_logs, st.session_state['user_name'], "Internal_Use", f"{sku} -{q} | {who} | {rsn} | {n}")
                        st.cache_data.clear(); st.rerun()
